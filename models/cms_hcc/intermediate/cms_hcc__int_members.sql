    ...
    */
    select
        ...
        , case
            ...
            when s.final_start > cast({{ concat_custom(['c.collection_year', "'-12-31'"]) }} as date) then 0
            /* Count inclusive months between max(PartBStart, Jan 1 of collection year) and Dec 31 of collection year */
            else ( {{ datediff(
                        'case when s.final_start > cast(' ~ concat_custom(['c.collection_year', "'-01-01'"]) ~ ' as date) then s.final_start else cast(' ~ concat_custom(['c.collection_year', "'-01-01'"]) ~ ' as date) end',
                        'cast(' ~ concat_custom(['c.collection_year', "'-12-31'"]) ~ ' as date)',
                        'month') }} + 1 )
          end as coverage_months
        , 12 as collection_months
    from (
        /* use full person-month grid to allow pre-eligibility months */
        select distinct person_id, payment_year, collection_year, collection_end_date
        from person_months
    ) c
    left join start s
